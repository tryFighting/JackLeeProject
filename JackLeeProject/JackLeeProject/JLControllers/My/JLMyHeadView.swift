...
import UIKit
protocol JLMyHeadViewDelegate:NSObjectProtocol {
    func shopBtnClick(tag: Int)
}
///我的页面的顶部的headView
class JLMyHeadView: UIView {
 
    weak var delegate: JLMyHeadViewDelegate?
    
    ///下面浮动的vip标签的view
   private lazy var animationView: JLMyVipView = {
        let view = JLMyVipView()
        return view
    }()
    ...
    func setupShopView(){
        let margin:CGFloat = UIScreen.main.bounds.width / 10
    ...
    ///vip开始动画
    func setAnimationVipView() -> () {
        let yoring: CGFloat = 100.0 + 64
        ...
        UIView.animate(withDuration: 1, delay: 1, options: opts, animations: {
            ...
            self.animationView.center.y = yoring
        }
    }
    ///vip停止动画
    func stopAnimation(){
